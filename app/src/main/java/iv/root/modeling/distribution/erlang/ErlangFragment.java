...
public class ErlangFragment extends Fragment implements FragmentAPI {
    private static final String ARGS_X0 = "args:x0";
    private static final String ARGS_XN = "args:xn";
    private EditText inputK;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_erlang_layout, container, false);

        inputK = view.findViewById(R.id.inputK);
    ...
    public static ErlangFragment getInstance(Double x0, Double xn) {
        ErlangFragment fragment = new ErlangFragment();
    ...
    public DataPoint[] getPoints_F(int quality) {
        String strK = inputK.getText().toString();
        String strLambda = inputLambda.getText().toString();

        if (!strK.isEmpty() && !strLambda.isEmpty()) {
            Integer K = Integer.valueOf(strK);
            Double lambda = Double.valueOf(strLambda);
            ErlangDistribution distribution = new ErlangDistribution(K, lambda);
